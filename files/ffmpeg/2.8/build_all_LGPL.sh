...
$NDK/build/tools/make-standalone-toolchain.sh --toolchain=x86-4.9 --arch=x86 --system=linux-x86_64 --platform=android-14 --install-dir=$TOOLCHAIN
...
LDFLAGS_PIE='-Wl,-z,relro -Wl,-z,now -pie -fpic'
...
export PATH=$TOOLCHAIN/bin:$PATH
export CC="ccache i686-linux-android-gcc-4.9"
...
cp -v $BUILD_DIRarm/bin/ffmpeg 			builds_LGPL/ffmpeg_arm
cp -v $BUILD_DIRarm_nonNEON/bin/ffmpeg 		builds_LGPL/ffmpeg_arm_nonNEON
cp -v $BUILD_DIRx86/bin/ffmpeg 			builds_LGPL/ffmpeg_x86
cp -v $BUILD_DIRarm_nonPIE/bin/ffmpeg 		builds_LGPL/ffmpeg_arm_nonPIE
cp -v $BUILD_DIRarm_nonNEON_nonPIE/bin/ffmpeg 	builds_LGPL/ffmpeg_arm_nonNEON_nonPIE
